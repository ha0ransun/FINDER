...
class FINDER:
    ...
    def __init__(self):
        ...
        self.g_type =  args.g_type#'barabasi_albert', 'small-world'
...
#################################################New code for FINDER#####################################
    ...
    def findModel(self):
        ...
        best_model = './models/nrange_%d_%d_iter_%d_%s_%s.ckpt' % (NUM_MIN, NUM_MAX, best_model_iter, self.g_type, self.training_type)
        return best_model
    ...
    def GenNetwork(self, g):    #networkx2four
        ...
        for i in range(len(nodes)):
            weights.append(g.nodes[str(i)]['weight'])
        if len(edges) > 0:
            a, b = zip(*edges)
            A = np.array(a)
            B = np.array(b)
    ...
    def HXA(self, g, method):
        ...
        return sol # return Robustness, sol
